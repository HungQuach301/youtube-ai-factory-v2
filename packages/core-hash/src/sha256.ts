...
export class Sha256 {
  ...
  update(data: Uint8Array): this {
    ...
    if (this.#bytesHashed + data.byteLength > Number.MAX_SAFE_INTEGER) {
      throw new RangeError('SHA-256 input exceeds the supported safe integer length')
    }
    this.#bytesHashed += data.byteLength

    let offset = 0
    while (offset < data.byteLength) {
      const available = this.#buffer.byteLength - this.#bufferLength
      const take = Math.min(available, data.byteLength - offset)
