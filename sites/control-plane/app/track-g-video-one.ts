...
import {
  ...
  stage12CodecSafeLraFeasibilitySearchEvidence,
  stage12CodecSafeLraFeasibilitySearchJobs,
...
import {
  ...
  listImmutableProductionEvidence,
  readVerifiedProductionEvidence,
  sha256,
...
import {
  ...
  parseStage12CodecSafeLraFeasibilitySearchResult,
  ...
  stage12CodecSafeLraFeasibilitySearchFingerprints,
  ...
  STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SOURCE_SHA256,
  STAGE12_CODEC_SAFE_LRA_GUARD_CONTROLLER_POLICY,
  type Stage12CodecSafeLraFeasibilitySearchResult,
  type Stage12CodecSafeLraGuardShadowResult,
...
const STAGE_10_CODE = "10";
const STAGE_10_STANDARD_VERSION = 1;
const STAGE_10_INSTANCE_ID = "stage_track_g_video_1_10_attempt_1";
...
const STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_OWNER_APPROVAL_TEXT =
...
export type RunTrackGVideoOneStage12CodecSafeLraFeasibilitySearchInput = {
  objective: string;
  ownerApprovalText: typeof STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_OWNER_APPROVAL_TEXT;
  callbackUrl: string;
  objectAccessUrl: string;
};
...
async function readBackStage10(operationRunId: string) {
  ...
  const db = getDb();
  const [stage] = await db.select().from(stageInstances)
    .where(eq(stageInstances.id, STAGE_10_INSTANCE_ID)).limit(1);
  const [artifact] = await db.select().from(stageArtifacts)
    .where(eq(stageArtifacts.id, STAGE_10_ARTIFACT_ID)).limit(1);
  const [production] = await db.select().from(stage10AudioProductions)
    .where(eq(stage10AudioProductions.id, STAGE_10_PRODUCTION_ID)).limit(1);
  const ceilings = await db.select().from(spendCeilings);
  const stageCeiling = ceilings.find((value) => value.scope === "STAGE"
    && value.scopeRef === STAGE_10_INSTANCE_ID)?.ceilingUsd;
  if (!stage || !artifact || !production || stage.controlState !== "FROZEN"
    || stage.packageId !== STAGE_00_PACKAGE_ID || stage.stageCode !== STAGE_10_CODE
    || stage.standardVersion !== STAGE_10_STANDARD_VERSION
    || artifact.stageInstanceId !== stage.id || artifact.artifactType !== STAGE_10_ARTIFACT_TYPE
    || artifact.namespace !== "production" || artifact.immutabilityState !== "SEALED"
    || artifact.eligibilityState !== "ELIGIBLE_FOR_STAGE"
    || artifact.standardVersion !== STAGE_10_STANDARD_VERSION
    || production.packageId !== STAGE_00_PACKAGE_ID || production.stageInstanceId !== stage.id
    || production.provider !== "ELEVENLABS" || production.providerCallCount !== 12
    || production.reservedUsd !== STAGE_10_RESERVED_USD
    || production.actualUsd < 0 || production.actualUsd > production.reservedUsd
    || stageCeiling !== STAGE_10_RESERVED_USD
    || !isAtOrAfterReadyStep(stage09.base.run.currentStep, "STAGE_11_READY")
    || !await verifyImmutableEvidence(stage09.stage09Artifact.r2Key,
      stage09.stage09Artifact.canonicalHash)
    || !await verifyImmutableEvidence(production.narrationR2Key, production.narrationSha256)
    || !await verifyImmutableEvidence(production.evidenceR2Key, production.evidenceSha256)
    || artifact.r2Key !== production.evidenceR2Key
    || artifact.canonicalHash !== production.evidenceSha256) {
    throw new Error("TRACK_G_STAGE_10_READ_BACK_FAILED");
  }
  return { ...stage09, stage10: stage, stage10Artifact: artifact,
    stageArtifact: artifact, production,
    gateResults: [
      { gate: "M1_PHONEME_MISMATCH", state: "PASS" as const,
        evidence: `Independent calibrated evidence ${production.calibrationEvidenceSha256}.` },
      { gate: "M1_SEAM_SCORE", state: "PASS" as const,
        evidence: `Narration ${production.narrationSha256} passed deterministic seam preflight.` },
    ] };
}
...
export async function finalizeTrackGVideoOneStage10(
  ...
  if (workerResult.totalCharacters !== totalCharacters
    ...
    || workerResult.champions.some((candidate) => !candidate.eligible
      || candidate.phonemeMismatchRate > workerResult.calibration.threshold)
    || workerResult.narration.seamScore > workerResult.narration.seamThreshold) {
    throw new Error("TRACK_G_STAGE_10_MEDIA_GATES_FAILED");
  }
  const allCandidates = [...workerResult.champions, ...workerResult.rejected];
  const candidateEvidence = [];
  for (const candidate of allCandidates) {
    const audio = new Uint8Array(Buffer.from(candidate.audioBase64, "base64"));
    if (sha256(audio) !== candidate.audioSha256) {
      throw new Error("TRACK_G_STAGE_10_CANDIDATE_AUDIO_HASH_FAILED");
    }
    const r2Key = ["prod", approvedChannel.id, trackGVideoOneContract.episodeId,
      STAGE_10_CODE, "takes", candidate.takeId, `${candidate.audioSha256}.mp3`].join("/");
    await putImmutableProductionEvidence(r2Key, audio, "audio/mpeg", candidate.audioSha256);
    candidateEvidence.push({
      takeId: candidate.takeId, segmentId: candidate.segmentId, route: candidate.route,
      seed: candidate.seed, r2Key, audioSha256: candidate.audioSha256,
      ...
      selected: workerResult.champions.some((value) => value.takeId === candidate.takeId),
    });
  }
  const narrationBytes = new Uint8Array(Buffer.from(workerResult.narration.audioBase64, "base64"));
  if (sha256(narrationBytes) !== workerResult.narration.audioSha256) {
    throw new Error("TRACK_G_STAGE_10_NARRATION_AUDIO_HASH_FAILED");
  }
  const narrationR2Key = ["prod", approvedChannel.id, trackGVideoOneContract.episodeId,
    STAGE_10_CODE, "narration", `${workerResult.narration.audioSha256}.mp3`].join("/");
  await putImmutableProductionEvidence(narrationR2Key, narrationBytes, "audio/mpeg",
    workerResult.narration.audioSha256);
  const actualUsd = Number(Math.min(STAGE_10_RESERVED_USD,
    workerResult.totalCharacters * qualifiedVoice.settings.usdPer1000Chars / 1000).toFixed(4));
  const evidenceEnvelope = {
    schemaVersion: 1,
    runnerContractVersion: 1,
    executorVersion: "stage-10-calibrated-narration-tournament-v1",
    operationRunId: bootstrap.run.id,
    packageId: STAGE_00_PACKAGE_ID,
    stageCode: STAGE_10_CODE,
    artifactType: STAGE_10_ARTIFACT_TYPE,
    predecessor: { artifactId: STAGE_09_ARTIFACT_ID,
      sha256: stage09.stage09Artifact.canonicalHash },
    voice: { provider: "ELEVENLABS", voiceId: qualifiedVoice.voiceId,
      modelId: qualifiedVoice.model, settingsHash: qualifiedVoice.settingsHash,
      selectedToneId: tone.id },
    calibration: workerResult.calibration,
    workerImageDigest: workerResult.imageDigest,
    tournament: { candidatesPerSegment: 2, segmentCount: 6,
      preserveRejectedCandidates: true, candidates: candidateEvidence },
    narration: { r2Key: narrationR2Key, sha256: workerResult.narration.audioSha256,
      seamScore: workerResult.narration.seamScore,
      seamThreshold: workerResult.narration.seamThreshold },
    gateResults: workerResult.gateResults,
    controls: { dispatchMode: "EXPLICIT_OWNER_COMMAND", providerDispatch: "OFF",
      releaseEligible: false, autoPublish: "OFF" },
    budget: { reservedUsd: STAGE_10_RESERVED_USD, actualUsd,
      providerCallCount: workerResult.providerCallCount,
      totalCharacters: workerResult.totalCharacters,
      rateUsdPer1000Chars: qualifiedVoice.settings.usdPer1000Chars },
  };
  const evidenceBytes = new TextEncoder().encode(`${canonicalize(evidenceEnvelope)}\n`);
  const evidenceSha256 = sha256(evidenceBytes);
  const evidenceR2Key = ["prod", approvedChannel.id, trackGVideoOneContract.episodeId,
    STAGE_10_CODE, "narration-calibration-seal", `${evidenceSha256}.json`].join("/");
  await putImmutableProductionEvidence(evidenceR2Key, evidenceBytes, "application/json",
    evidenceSha256);
  const [latestEvent] = await db.select({ ordinal: operationEvents.ordinal }).from(operationEvents)
    .where(eq(operationEvents.runId, bootstrap.run.id)).orderBy(desc(operationEvents.ordinal)).limit(1);
  const firstOrdinal = (latestEvent?.ordinal ?? 0) + 1;
  ...
  try {
    await d1.batch([
      d1.prepare(`INSERT INTO command_log
        ...
        providerCallCount: workerResult.providerCallCount, actualUsd, evidenceSha256,
      }), input.idempotencyKey, user.email.toLowerCase(), traceId, now),
      d1.prepare(`INSERT INTO stage_instance
        (id, package_id, stage_code, control_state, standard_version, attempt_ordinal, started_at, frozen_at)
        VALUES (?, ?, '10', 'FROZEN', ?, 1, ?, ?)`).bind(
        STAGE_10_INSTANCE_ID, STAGE_00_PACKAGE_ID, STAGE_10_STANDARD_VERSION, now, now),
      d1.prepare(`INSERT INTO stage_artifact
        (id, stage_instance_id, artifact_type, namespace, r2_key, canonical_hash,
         immutability_state, eligibility_state, standard_version, created_at)
        VALUES (?, ?, ?, 'production', ?, ?, 'SEALED', 'ELIGIBLE_FOR_STAGE', ?, ?)`).bind(
        STAGE_10_ARTIFACT_ID, STAGE_10_INSTANCE_ID, STAGE_10_ARTIFACT_TYPE,
        evidenceR2Key, evidenceSha256, STAGE_10_STANDARD_VERSION, now),
      d1.prepare(`INSERT INTO stage10_audio_production
        (id, package_id, stage_instance_id, idempotency_key, provider, provider_call_count,
         total_characters, reserved_usd, actual_usd, calibration_evidence_sha256,
        ...
        evidenceR2Key, evidenceSha256, now),
      d1.prepare(`INSERT INTO spend_ceiling (scope, scope_ref, ceiling_usd)
        VALUES ('STAGE', ?, ?)`).bind(STAGE_10_INSTANCE_ID, STAGE_10_RESERVED_USD),
      d1.prepare(`UPDATE operation_run SET current_step = 'STAGE_11_READY', updated_at = ?
        WHERE id = ? AND status = 'RUNNING' AND current_step = 'STAGE_10_READY'`).bind(
        now, bootstrap.run.id),
      ...[
        ["STAGE_10_DOR_PASSED", { predecessor: STAGE_09_ARTIFACT_ID,
          predecessorSha256: stage09.stage09Artifact.canonicalHash }],
        ["STAGE_10_PROVIDER_BUDGET_RESERVED", { reservedUsd: STAGE_10_RESERVED_USD,
          maxProviderCalls: 12, autoDispatch: "OFF" }],
        ["STAGE_10_CALIBRATED_TOURNAMENT_PASSED", { calibration: workerResult.calibration,
          gates: workerResult.gateResults }],
        ["STAGE_10_REJECTED_TAKES_PRESERVED", { rejectedCount: workerResult.rejected.length }],
        ["STAGE_10_ARTIFACT_SEALED", { artifactId: STAGE_10_ARTIFACT_ID,
          evidenceR2Key, evidenceSha256, narrationR2Key,
          narrationSha256: workerResult.narration.audioSha256 }],
        ["STAGE_10_FROZEN", { nextStep: "STAGE_11_READY",
          reservedUsd: STAGE_10_RESERVED_USD, actualUsd, providerDispatch: "OFF",
          autoPublish: "OFF" }],
      ].map(([eventType, payload], index) => d1.prepare(`INSERT INTO operation_event
        (id, run_id, ordinal, event_type, payload_json, created_at) VALUES (?, ?, ?, ?, ?, ?)`).bind(
        crypto.randomUUID(), bootstrap.run.id, firstOrdinal + index, eventType,
    ...
    const [concurrent] = await db.select({ id: commandLog.id }).from(commandLog)
      .where(eq(commandLog.idempotencyKey, input.idempotencyKey)).limit(1);
    if (concurrent) return { ...(await readBackStage10(bootstrap.run.id)), replayed: true };
    throw error;
  }
...
function stage12RecoveryIdempotencyKey(workerIdempotencyKey: string, preMasterSha256: string): string {
  return createHash("sha256").update(
    `RECOVER_TRACK_G_VIDEO_1_STAGE_12_ATTEMPT_3\0${workerIdempotencyKey}\0${preMasterSha256}`,
  ).digest("hex");
}
...
function stage12DiagnosticIdempotencyKey(
  ...
  return createHash("sha256").update(
    `SCAN_TRACK_G_VIDEO_1_STAGE_12_ATTEMPT_3\0${workerIdempotencyKey}\0${preMasterSha256}\0${diagnosticOrdinal}`,
  ).digest("hex");
}
...
function stage12PreMasterPrefix(): string {
  return ["prod", approvedChannel.id, trackGVideoOneContract.episodeId,
    STAGE_12_CODE, "pre-master", ""].join("/");
}

async function stage12PreMasterCandidates() {
...
async function verifyStage12DiagnosticPreMasterPointer(
  preMaster: Stage12MediaReceipt["preMaster"],
): Promise<boolean> {
  const candidate = (await stage12PreMasterCandidates()).find((object) =>
    object.key === preMaster.r2Key && object.sha256 === preMaster.sha256
...
async function latestStage12QaDiagnosticJob() {
  const [job] = await getDb().select().from(stage12QaDiagnosticJobs)
    .orderBy(desc(stage12QaDiagnosticJobs.diagnosticOrdinal)).limit(1);
  return job;
}
...
export async function diagnoseTrackGVideoOneStage12AttemptThreeQa() {
  ...
  return {
    ...
    diagnosticOrdinal: diagnosticJob?.diagnosticOrdinal ?? null,
    retryOfDiagnosticJobId: diagnosticJob?.retryOfDiagnosticJobId ?? null,
    retryReasonCode: diagnosticJob?.retryReasonCode ?? null,
...
export async function diagnoseTrackGVideoOneStage12Recovery() {
  ...
  const eligible = job?.attemptOrdinal === 3 && job.state === "FAILED"
    && ["STAGE12_CALLBACK_FAILED", "STAGE12_CALLBACK_FAILED:422"].includes(job.errorCode ?? "")
    && candidates.length === 1;
  return {
    recoveryState: eligible ? "PASS" as const : "FAIL" as const,
    errorCode: eligible ? null : !job
      ? "TRACK_G_STAGE_12_JOB_READ_BACK_FAILED"
      : job.attemptOrdinal !== 3
        ? "TRACK_G_STAGE_12_RECOVERY_REQUIRES_ATTEMPT_3"
        : job.state !== "FAILED"
          ? `TRACK_G_STAGE_12_RECOVERY_JOB_${job.state}`
          : candidates.length !== 1
            ? `TRACK_G_STAGE_12_RECOVERY_PRE_MASTER_COUNT:${candidates.length}`
            : `TRACK_G_STAGE_12_RECOVERY_ERROR_NOT_ELIGIBLE:${job.errorCode ?? "UNKNOWN"}`,
    attemptOrdinal: job?.attemptOrdinal ?? null,
    jobStatus: job?.state ?? "NONE",
    preMasterCandidates: candidates,
    providerDispatch: "OFF" as const,
    autoPublish: "OFF" as const,
  };
}
...
export async function readTrackGVideoOneStage12PreMaster(
  idempotencyKey: string,
  token: string,
  ...
  const job = await latestStage12Job();
  requireStage12WorkerToken(job, idempotencyKey, token);
  if (job.state !== "PENDING" || job.attemptOrdinal !== 3 || !HEX64.test(expectedSha256)) {
    throw new Error("STAGE_12_RECOVERY_STATE_CONFLICT");
  }
  const candidate = (await stage12PreMasterCandidates())
    .find((object) => object.sha256 === expectedSha256);
  if (!candidate) throw new Error("STAGE_12_RECOVERY_PRE_MASTER_NOT_FOUND");
...
export async function recordTrackGVideoOneStage12DiagnosticCallback(input: {
  ...
  if (input.result === undefined) {
    const errorCode = input.errorCode && /^[A-Z][A-Z0-9_:.-]{0,159}$/u.test(input.errorCode)
  ...
  if (diagnosticJob.targetDurationSec === null || diagnosticJob.targetDurationSec <= 0) {
    throw new Error("STAGE_12_QA_DIAGNOSTIC_DURATION_MISSING");
  }
  const evaluation = evaluateTrackGVideoOneStage12Receipt(
    parsed, diagnosticJob.targetDurationSec,
  );
  if (!verifyStage12MeasurementReport(parsed)
    || !await verifyStage12DiagnosticPreMasterPointer(parsed.preMaster)) {
...
export async function recoverTrackGVideoOneStage12AttemptThree(
  user: ChatGPTUser,
  input: RecoverTrackGVideoOneStage12Input,
) {
  if (input.ownerApprovalText !== STAGE_12_RECOVERY_OWNER_APPROVAL_TEXT) {
    throw new Error("TRACK_G_STAGE_12_RECOVERY_OWNER_APPROVAL_REQUIRED");
  }
  const objective = input.objective.trim();
  ...
  const job = await latestStage12Job();
  if (!job || job.attemptOrdinal !== 3) {
    throw new Error("TRACK_G_STAGE_12_RECOVERY_REQUIRES_ATTEMPT_3");
  }
  const prepared = await prepareStage12MediaRequest(3);
  if (job.idempotencyKey !== prepared.workerIdempotencyKey) {
    throw new Error("TRACK_G_STAGE_12_JOB_READ_BACK_FAILED");
  }
  const candidates = await stage12PreMasterCandidates();
  if (candidates.length !== 1) {
    throw new Error(`TRACK_G_STAGE_12_RECOVERY_PRE_MASTER_COUNT:${candidates.length}`);
  }
  ...
  if (job.state !== "FAILED" || !["STAGE12_CALLBACK_FAILED", "STAGE12_CALLBACK_FAILED:422"]
  ...
  const results = await d1.batch([
    d1.prepare(`INSERT INTO command_log
      (id, command_type, payload_json, idempotency_key, actor_identity, prev_state, next_state,
       trace_id, created_at) VALUES (?, 'RECOVER_TRACK_G_VIDEO_1_STAGE_12_ATTEMPT_3', ?, ?, ?,
       'TRACK_G_VIDEO_1_STAGE_12_FAILED', 'TRACK_G_VIDEO_1_STAGE_12_PENDING', ?, ?)`).bind(
      crypto.randomUUID(), canonicalize({ objective, operationRunId: prepared.bootstrap.run.id,
        packageId: STAGE_00_PACKAGE_ID, stageCode: STAGE_12_CODE, attemptOrdinal: 3,
        workerIdempotencyKey: job.idempotencyKey, preMaster }), recoveryKey,
      user.email.toLowerCase(), crypto.randomUUID(), now),
    d1.prepare(`UPDATE stage12_media_job SET state = 'PENDING', error_code = NULL,
      updated_at = ? WHERE id = ? AND attempt_ordinal = 3 AND state = 'FAILED'`).bind(now, job.id),
  ]);
  if ((results[1]?.meta.changes ?? 0) !== 1) {
    throw new Error("TRACK_G_STAGE_12_RECOVERY_CONCURRENT_STATE_CONFLICT");
  }
...
export async function scanTrackGVideoOneStage12AttemptThree(
  ...
  let diagnosticOrdinal = 1;
  let retryOfDiagnosticJobId: string | null = null;
  let retryReasonCode: string | null = null;
  if (existing) {
    if (existing.stage12JobId !== sourceJob.id) {
      throw new Error("TRACK_G_STAGE_12_DIAGNOSTIC_JOB_SOURCE_MISMATCH");
    }
    const retryableCallbackFailure = existing.state === "FAILED"
      && existing.diagnosticOrdinal === 1
      && ["23", "STAGE12_CALLBACK_TIMEOUT"].includes(existing.errorCode ?? "");
    if (!retryableCallbackFailure) {
      return { ...(await diagnoseTrackGVideoOneStage12AttemptThreeQa()), replayed: true };
    }
    diagnosticOrdinal = 2;
    retryOfDiagnosticJobId = existing.id;
    retryReasonCode = "STAGE12_DIAGNOSTIC_CALLBACK_TIMEOUT";
  }
  const prepared = await prepareStage12MediaRequest(3);
  const diagnosticKey = stage12DiagnosticIdempotencyKey(
    sourceJob.idempotencyKey, preMaster.sha256, diagnosticOrdinal,
  );
  const callbackToken = stage12DiagnosticCallbackToken(diagnosticKey);
  const diagnosticId = diagnosticOrdinal === 1
    ? `stage12_qa_diagnostic_${sourceJob.id}`
    : `stage12_qa_diagnostic_${sourceJob.id}_retry_${diagnosticOrdinal}`;
  const now = new Date().toISOString();
  await getD1().batch([
    getD1().prepare(`INSERT INTO command_log
      ...
      crypto.randomUUID(), canonicalize({ objective, sourceJobId: sourceJob.id,
        attemptOrdinal: 3, diagnosticOrdinal, retryOfDiagnosticJobId,
        retryReasonCode, preMaster, generation: false, providerDispatch: "OFF",
    ...
    getD1().prepare(`INSERT INTO stage12_qa_diagnostic_job
      (id, stage12_job_id, idempotency_key, callback_token_hash, state,
       diagnostic_ordinal, retry_of_diagnostic_job_id, retry_reason_code,
       target_duration_sec, created_at, updated_at)
      VALUES (?, ?, ?, ?, 'PENDING', ?, ?, ?, ?, ?, ?)`).bind(
      diagnosticId, sourceJob.id, diagnosticKey,
      sha256(new TextEncoder().encode(callbackToken)), diagnosticOrdinal,
      retryOfDiagnosticJobId, retryReasonCode, prepared.payload.durationSec, now, now,
    ),
  ]);
  try {
    ...
    const errorCode = error instanceof Error
      ...
      ? error.message : "STAGE12_DIAGNOSTIC_START_FAILED";
    await getD1().prepare(`UPDATE stage12_qa_diagnostic_job SET state = 'FAILED',
      error_code = ?, updated_at = ? WHERE id = ? AND state = 'PENDING'`).bind(
      errorCode,
...
function stage12AudioP0CorrectionCallbackToken(idempotencyKey: string) {
  const signingKey = getFactoryEnv().MEDIA_REQUEST_SIGNING_KEY;
  if (!signingKey) throw new Error("MEDIA_REQUEST_SIGNING_KEY_UNAVAILABLE");
  return createHash("sha256").update(
    `STAGE_12_AUDIO_P0_CORRECTION_CALLBACK\0${idempotencyKey}\0${signingKey}`,
  ).digest("hex");
}
...
async function stage12AudioP0CorrectionSource() {
  ...
  const pointer = (await listImmutableProductionEvidence(prefix, 8)).find((object) =>
    object.key === predecessor.correctedPreMasterR2Key
      ...
      && object.size === predecessor.correctedPreMasterByteLength);
  const eligible = Boolean(pointer && predecessor.state === "READY" && predecessor.outcome === "FAIL"
    && predecessor.providerCallCount === 0 && predecessor.providerDispatch === "OFF"
    && predecessor.autoPublish === "OFF"
    && failures.includes("TECHNICAL_DEFECT") && failures.includes("LOUDNESS")
    && failures.includes("M0_INPUT_RIGHTS_P0")
    && measurements.clippingSampleCount > 0
...
function requireStage12AudioP0CorrectionToken(
  job: Awaited<ReturnType<typeof latestStage12AudioP0CorrectionJob>>,
  idempotencyKey: string, token: string,
): asserts job is NonNullable<typeof job> {
  if (!job || job.idempotencyKey !== idempotencyKey || !HEX64.test(token)
    || job.callbackTokenHash !== sha256(new TextEncoder().encode(token))) {
    throw new Error("STAGE_12_AUDIO_P0_CORRECTION_UNAUTHORIZED");
  }
}

export async function readTrackGVideoOneStage12AudioP0CorrectionSource(
  idempotencyKey: string, token: string, expectedSha256: string,
) {
  const job = await latestStage12AudioP0CorrectionJob();
  requireStage12AudioP0CorrectionToken(job, idempotencyKey, token);
  if (job.state !== "PENDING" || expectedSha256 !== job.sourcePreMasterSha256) {
    throw new Error("STAGE_12_AUDIO_P0_CORRECTION_SOURCE_CONFLICT");
  }
  return readVerifiedProductionEvidence(job.sourcePreMasterR2Key, job.sourcePreMasterSha256);
}

export async function storeTrackGVideoOneStage12AudioP0CorrectedPreMaster(
  idempotencyKey: string, token: string, bytes: Uint8Array, expectedSha256: string,
) {
  const job = await latestStage12AudioP0CorrectionJob();
  requireStage12AudioP0CorrectionToken(job, idempotencyKey, token);
  if (job.state !== "PENDING" || !HEX64.test(expectedSha256) || sha256(bytes) !== expectedSha256
    || expectedSha256 === job.sourcePreMasterSha256) {
    throw new Error("STAGE_12_AUDIO_P0_CORRECTION_UPLOAD_INVALID");
  }
  const r2Key = ["prod", approvedChannel.id, trackGVideoOneContract.episodeId,
    STAGE_12_CODE, "audio-p0-corrected-pre-master", `${expectedSha256}.webm`].join("/");
  await putImmutableProductionEvidence(r2Key, bytes, "video/webm", expectedSha256);
  return { r2Key, sha256: expectedSha256, byteLength: bytes.byteLength };
}
...
export async function recordTrackGVideoOneStage12AudioP0CorrectionCallback(input: {
  ...
  if (job.state === "READY") return { accepted: true, replayed: true, jobStatus: "READY" as const };
  if (job.state !== "PENDING") throw new Error("STAGE_12_AUDIO_P0_CORRECTION_CALLBACK_STATE_CONFLICT");
  if (!input.result) {
    const code = input.errorCode && /^[A-Z0-9_:.-]{1,160}$/u.test(input.errorCode)
  ...
  if (!parsed.preMaster.r2Key.startsWith(correctedPrefix)
    || parsed.preMaster.sha256 === job.sourcePreMasterSha256
    || !verifyStage12MeasurementReport(parsed) || !correctedPointer) {
  ...
  if ((result.meta.changes ?? 0) !== 1) {
    throw new Error("STAGE_12_AUDIO_P0_CORRECTION_CONCURRENT_STATE_CONFLICT");
  }
  return { accepted: true, replayed: false, jobStatus: "READY" as const };
}

export async function createTrackGVideoOneStage12AudioP0Correction(
  user: ChatGPTUser, input: CreateTrackGVideoOneStage12AudioP0CorrectionInput,
) {
  if (input.ownerApprovalText !== STAGE_12_AUDIO_P0_CORRECTION_OWNER_APPROVAL_TEXT) {
    throw new Error("TRACK_G_STAGE_12_AUDIO_P0_CORRECTION_OWNER_APPROVAL_REQUIRED");
  }
  if (input.objective.trim().length < 12 || input.objective.trim().length > 500
    || !input.callbackUrl.startsWith("https://") || !input.objectAccessUrl.startsWith("https://")) {
  ...
  try {
    await dispatchStage12MediaAudioP0Correction({ ...prepared.payload, idempotencyKey: key,
      ...
      remediation: { sourceAttemptOrdinal: 3, diagnosticOrdinal: 2, strategyVersion,
        ...
        sourceCorrectionReceiptSha256: predecessor.receiptSha256,
...
function stage12CodecSafeLraGuardShadowIdempotencyKey(
  sourceSha256: string,
  sourceReceiptSha256: string,
  ...
  return createHash("sha256").update(
    ["RUN_TRACK_G_VIDEO_1_STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_REPLAY",
      sourceSha256, sourceReceiptSha256, parentShadowEvidenceId,
      expectedWorkerImageDigest, algorithmFingerprint].join("\0"),
  ).digest("hex");
}

function stage12CodecSafeLraGuardShadowCallbackToken(idempotencyKey: string) {
  const signingKey = getFactoryEnv().MEDIA_REQUEST_SIGNING_KEY;
  if (!signingKey) throw new Error("MEDIA_REQUEST_SIGNING_KEY_UNAVAILABLE");
  return createHash("sha256").update(
    `STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_CALLBACK\0${idempotencyKey}\0${signingKey}`,
  ).digest("hex");
}

async function latestStage12CodecSafeLraGuardShadowJob() {
  const [job] = await getDb().select().from(stage12CodecSafeLraGuardShadowJobs)
    .orderBy(desc(stage12CodecSafeLraGuardShadowJobs.createdAt)).limit(1);
  return job ?? null;
}

async function readStage12CodecSafeLraGuardShadowEvidence(shadowJobId: string) {
  const [evidence] = await getDb().select().from(stage12CodecSafeLraGuardShadowEvidence)
    .where(eq(stage12CodecSafeLraGuardShadowEvidence.shadowJobId, shadowJobId)).limit(1);
  return evidence ?? null;
}

async function stage12CodecSafeLraGuardShadowSource() {
  const lineage = await stage12CodecSafeTruePeakShadowSource();
  const parentShadowJob = await latestStage12CodecSafeTruePeakShadowJob();
  const parentShadowEvidence = parentShadowJob
    ? await readStage12CodecSafeTruePeakShadowEvidence(parentShadowJob.id) : null;
  const parentShadowResult = parentShadowEvidence
    ? stage12CodecSafeTruePeakShadowEvidenceResult(parentShadowEvidence) : null;
  const anchorReference = parentShadowResult?.candidates.find(
    (candidate) => candidate.candidatePass === 1,
  ) ?? null;
  const highBracketReference = parentShadowResult?.candidates.find(
    (candidate) => candidate.candidatePass === 3,
  ) ?? null;
  const eligible = Boolean(lineage.eligible && lineage.source && lineage.historicalFailure
    && lineage.diagnosticReplayJob && lineage.diagnosticReplayEvidence
    && parentShadowJob && parentShadowEvidence && parentShadowResult
    && parentShadowJob.state === "READY" && parentShadowJob.shadowOutcome === "FAIL"
    && parentShadowResult.shadowOutcome === "FAIL"
    && parentShadowJob.sourceCorrectionJobId === lineage.source.id
    && parentShadowJob.historicalFailureJobId === lineage.historicalFailure.id
    && parentShadowJob.diagnosticReplayJobId === lineage.diagnosticReplayJob.id
    && parentShadowJob.diagnosticReplayEvidenceId === lineage.diagnosticReplayEvidence.id
    && parentShadowEvidence.sourcePreMasterSha256
      === lineage.source.correctedPreMasterSha256
    && parentShadowEvidence.sourceReceiptSha256 === lineage.source.receiptSha256
    && parentShadowEvidence.workerImageDigest
      === parentShadowEvidence.expectedWorkerImageDigest
    && parentShadowEvidence.correctedOutputUploaded === 0
    && parentShadowEvidence.historicalBackfill === 0
    && parentShadowEvidence.providerCallCount === 0
    && parentShadowEvidence.providerDispatch === "OFF"
    && parentShadowEvidence.calibrationExecuted === 0
    && parentShadowEvidence.finalizeExecuted === 0
    && parentShadowEvidence.releaseEligible === 0
    && parentShadowEvidence.productionActivationExecuted === 0
    && parentShadowEvidence.autoPublish === "OFF"
    && anchorReference?.truePeakDbtp !== undefined
    && anchorReference.truePeakDbtp <= AUDIO.TRUE_PEAK_MAX_DBTP
    && anchorReference.loudnessRangeLu < AUDIO.LRA.min
    && highBracketReference?.loudnessRangeLu !== undefined
    && highBracketReference.loudnessRangeLu > AUDIO.LRA.max
    && highBracketReference.macroDepthDb > anchorReference.macroDepthDb);
  return { ...lineage, parentShadowJob, parentShadowEvidence, parentShadowResult,
    anchorReference, highBracketReference, eligible };
}

function stage12CodecSafeLraGuardEvidenceResult(
  evidence: NonNullable<Awaited<ReturnType<
    typeof readStage12CodecSafeLraGuardShadowEvidence
  >>>,
) {
  return parseStage12CodecSafeLraGuardShadowResult({
    ...
    diagnosticReplay: { jobId: evidence.diagnosticReplayJobId,
      evidenceId: evidence.diagnosticReplayEvidenceId },
    parentShadow: { jobId: evidence.parentShadowJobId,
      evidenceId: evidence.parentShadowEvidenceId },
    losslessReference: { sha256: evidence.losslessReferenceSha256,
      byteLength: evidence.losslessReferenceByteLength,
      audioFrameMd5Sha256: evidence.losslessReferenceFrameMd5Sha256,
      codec: evidence.losslessReferenceCodec,
      sampleRateHz: evidence.losslessReferenceSampleRateHz },
    anchorReference: JSON.parse(evidence.anchorReferenceJson) as unknown,
    highBracketReference: JSON.parse(evidence.highBracketReferenceJson) as unknown,
    controllerPolicy: JSON.parse(evidence.controllerPolicyJson) as unknown,
    candidates: JSON.parse(evidence.candidatesJson) as unknown,
    shadowOutcome: evidence.shadowOutcome,
    terminalReason: evidence.terminalReason,
    lastEvaluatedCandidatePass: evidence.lastEvaluatedCandidatePass,
    bestSafeCandidatePass: evidence.bestSafeCandidatePass,
    selectedCandidatePass: evidence.selectedCandidatePass,
    finalMeasurements: { integratedLufs: evidence.finalIntegratedLufs,
      integratedLufsExact: evidence.finalIntegratedLufsExact,
      truePeakDbtp: evidence.finalTruePeakDbtp,
      truePeakDbtpExact: evidence.finalTruePeakDbtpExact,
    ...
    failedPredicates: JSON.parse(evidence.failedPredicatesJson) as unknown,
    expectedWorkerImageDigest: evidence.expectedWorkerImageDigest,
    parentWorkerImageDigest: evidence.parentWorkerImageDigest,
    workerImageDigest: evidence.workerImageDigest,
    algorithmFingerprint: evidence.algorithmFingerprint,
    thresholdSnapshotSha256: evidence.thresholdSnapshotSha256,
    controllerPolicySha256: evidence.controllerPolicySha256,
    renderKernelFingerprint: evidence.renderKernelFingerprint,
    parentRenderRuntimeFingerprint: evidence.parentRenderRuntimeFingerprint,
    renderRuntimeFingerprint: evidence.renderRuntimeFingerprint,
    parentRuntimeProvenance: JSON.parse(evidence.parentRuntimeProvenanceJson) as unknown,
    runtimeProvenance: JSON.parse(evidence.runtimeProvenanceJson) as unknown,
    correctedOutputUploaded: false,
    historicalBackfill: false,
    providerCallCount: 0,
    providerDispatch: "OFF",
...
export async function diagnoseTrackGVideoOneStage12CodecSafeLraGuardShadow() {
  const source = await stage12CodecSafeLraGuardShadowSource();
  const job = await latestStage12CodecSafeLraGuardShadowJob();
  const evidence = job ? await readStage12CodecSafeLraGuardShadowEvidence(job.id) : null;
  const result = evidence ? stage12CodecSafeLraGuardEvidenceResult(evidence) : null;
  if (job?.state === "READY" && (!result
    || result.workerImageDigest !== job.workerImageDigest
    || result.shadowOutcome !== job.shadowOutcome
    || result.terminalReason !== job.terminalReason
    || result.lastEvaluatedCandidatePass !== job.lastEvaluatedCandidatePass
    || result.bestSafeCandidatePass !== job.bestSafeCandidatePass
    || result.selectedCandidatePass !== job.selectedCandidatePass)) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_READ_BACK_FAILED");
  }
  return {
    shadowState: job?.state ?? (source.eligible ? "ELIGIBLE" : "BLOCKED"),
    errorCode: job?.errorCode ?? (source.eligible ? null
      : "STAGE12_CODEC_SAFE_LRA_GUARD_SHADOW_SOURCE_NOT_ELIGIBLE"),
    currentStep: source.bootstrap.run.currentStep,
    sourceAttemptOrdinal: 3 as const,
    sourceCorrectionOrdinal: 2 as const,
    historicalFailureCorrectionOrdinal: 3 as const,
    ...
    diagnosticReplayEvidenceId: job?.diagnosticReplayEvidenceId
      ?? source.diagnosticReplayEvidence?.id ?? null,
    parentShadowJobId: job?.parentShadowJobId ?? source.parentShadowJob?.id ?? null,
    parentShadowEvidenceId: job?.parentShadowEvidenceId
      ?? source.parentShadowEvidence?.id ?? null,
    sourcePreMasterSha256: job?.sourcePreMasterSha256
      ?? source.source?.correctedPreMasterSha256 ?? null,
    expectedWorkerImageDigest: job?.expectedWorkerImageDigest ?? null,
    parentWorkerImageDigest: job?.parentWorkerImageDigest
      ?? source.parentShadowEvidence?.workerImageDigest ?? null,
    workerImageDigest: job?.workerImageDigest ?? null,
    shadowOutcome: job?.shadowOutcome ?? null,
    terminalReason: job?.terminalReason ?? null,
    lastEvaluatedCandidatePass: job?.lastEvaluatedCandidatePass ?? null,
    bestSafeCandidatePass: job?.bestSafeCandidatePass ?? null,
    selectedCandidatePass: job?.selectedCandidatePass ?? null,
    result,
    evidenceSemantics: "CODEC_SAFE_LRA_GUARD_SHADOW_NOT_CORRECTION" as const,
    shadowExecuted: job !== null,
    correctedOutputUploaded: false as const,
    historicalBackfill: false as const,
    providerCallCount: 0 as const,
...
function requireStage12CodecSafeLraGuardShadowToken(
  job: Awaited<ReturnType<typeof latestStage12CodecSafeLraGuardShadowJob>>,
  idempotencyKey: string,
  token: string,
): asserts job is NonNullable<typeof job> {
  if (!job || job.idempotencyKey !== idempotencyKey || !HEX64.test(token)
    || job.callbackTokenHash !== sha256(new TextEncoder().encode(token))) {
    throw new Error("STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_UNAUTHORIZED");
  }
}

export async function readTrackGVideoOneStage12CodecSafeLraGuardShadowSource(
  idempotencyKey: string,
  token: string,
  expectedSha256: string,
) {
  const job = await latestStage12CodecSafeLraGuardShadowJob();
  requireStage12CodecSafeLraGuardShadowToken(job, idempotencyKey, token);
  if (job.state !== "PENDING" || expectedSha256 !== job.sourcePreMasterSha256) {
    throw new Error("STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_SOURCE_CONFLICT");
  }
  return readVerifiedProductionEvidence(job.sourcePreMasterR2Key, job.sourcePreMasterSha256);
}

export async function recordTrackGVideoOneStage12CodecSafeLraGuardShadowCallback(input: {
  idempotencyKey: string;
  token: string;
  result?: Stage12CodecSafeLraGuardShadowResult;
  errorCode?: string;
}) {
  const job = await latestStage12CodecSafeLraGuardShadowJob();
  requireStage12CodecSafeLraGuardShadowToken(job, input.idempotencyKey, input.token);
  if (job.state === "READY") {
    return { accepted: true, replayed: true, jobStatus: "READY" as const };
  }
  if (job.state !== "PENDING") {
    throw new Error("STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_CALLBACK_STATE_CONFLICT");
  }
  if (!input.result) {
    const code = input.errorCode && /^[A-Z0-9_:.-]{1,160}$/u.test(input.errorCode)
      ? input.errorCode : "STAGE12_CODEC_SAFE_LRA_GUARD_SHADOW_FAILED";
    await getD1().prepare(`UPDATE stage12_codec_safe_lra_guard_shadow_job
      SET state='FAILED',error_code=?,updated_at=? WHERE id=? AND state='PENDING'`)
      .bind(code, new Date().toISOString(), job.id).run();
    return { accepted: true, replayed: false, jobStatus: "FAILED" as const };
  }
  const result = parseStage12CodecSafeLraGuardShadowResult(input.result);
  if (result.source.correctionJobId !== job.sourceCorrectionJobId
    || result.historicalFailure.correctionJobId !== job.historicalFailureJobId
    || result.diagnosticReplay.jobId !== job.diagnosticReplayJobId
    || result.diagnosticReplay.evidenceId !== job.diagnosticReplayEvidenceId
    || result.parentShadow.jobId !== job.parentShadowJobId
    || result.parentShadow.evidenceId !== job.parentShadowEvidenceId
    || result.source.sha256 !== job.sourcePreMasterSha256
    || result.source.receiptSha256 !== job.sourceReceiptSha256
    || result.expectedWorkerImageDigest !== job.expectedWorkerImageDigest
    || result.parentWorkerImageDigest !== job.parentWorkerImageDigest
    || result.workerImageDigest !== job.expectedWorkerImageDigest
    || result.algorithmFingerprint !== job.algorithmFingerprint
    || result.thresholdSnapshotSha256 !== job.thresholdSnapshotSha256
    || result.controllerPolicySha256 !== job.controllerPolicySha256
    || result.renderKernelFingerprint !== job.renderKernelFingerprint
    || result.parentRenderRuntimeFingerprint !== job.parentRenderRuntimeFingerprint
    || result.renderRuntimeFingerprint !== job.parentRenderRuntimeFingerprint) {
    throw new Error("STAGE12_CODEC_SAFE_LRA_GUARD_SHADOW_RESULT_LINEAGE_INVALID");
  }
  const evidenceId = sha256(new TextEncoder().encode(canonicalize({
    shadowJobId: job.id, result,
  })));
  const now = new Date().toISOString();
  await getD1().batch([
    getD1().prepare(`INSERT INTO stage12_codec_safe_lra_guard_shadow_evidence
      (id,shadow_job_id,stage12_job_id,source_correction_job_id,historical_failure_job_id,
      ...
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'pcm_f32le',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,
       ?,?,?,?,?,?,?,?,?,?,?,?)`).bind(
      evidenceId, job.id, job.stage12JobId, job.sourceCorrectionJobId,
      job.historicalFailureJobId, job.diagnosticReplayJobId,
      job.diagnosticReplayEvidenceId, job.parentShadowJobId, job.parentShadowEvidenceId,
      result.evidenceSemantics, result.losslessReference.sha256,
      result.losslessReference.byteLength, result.losslessReference.audioFrameMd5Sha256,
      result.losslessReference.sampleRateHz, canonicalize(result.anchorReference),
      canonicalize(result.highBracketReference), canonicalize(result.controllerPolicy),
      canonicalize(result.candidates), result.lastEvaluatedCandidatePass,
      result.bestSafeCandidatePass, result.selectedCandidatePass, result.terminalReason,
      result.finalMeasurements.integratedLufs, result.finalMeasurements.integratedLufsExact,
      result.finalMeasurements.truePeakDbtp, result.finalMeasurements.truePeakDbtpExact,
      result.finalMeasurements.loudnessRangeLu, result.finalMeasurements.loudnessRangeLuExact,
      canonicalize(result.failedPredicates), result.shadowOutcome,
      result.expectedWorkerImageDigest, result.parentWorkerImageDigest,
      result.workerImageDigest, result.algorithmFingerprint,
      result.thresholdSnapshotSha256, result.controllerPolicySha256,
      result.renderKernelFingerprint, result.parentRenderRuntimeFingerprint,
      result.renderRuntimeFingerprint, canonicalize(result.parentRuntimeProvenance),
      canonicalize(result.runtimeProvenance), result.source.r2Key, result.source.sha256,
      result.source.byteLength, result.source.receiptSha256),
    getD1().prepare(`UPDATE stage12_codec_safe_lra_guard_shadow_job SET state='READY',
      shadow_outcome=?,terminal_reason=?,last_evaluated_candidate_pass=?,
      best_safe_candidate_pass=?,selected_candidate_pass=?,worker_image_digest=?,
      render_runtime_fingerprint=?,updated_at=? WHERE id=? AND state='PENDING'`).bind(
      result.shadowOutcome, result.terminalReason, result.lastEvaluatedCandidatePass,
      result.bestSafeCandidatePass, result.selectedCandidatePass, result.workerImageDigest,
      result.renderRuntimeFingerprint, now, job.id),
  ]);
  return { accepted: true, replayed: false, jobStatus: "READY" as const };
}

export async function runTrackGVideoOneStage12CodecSafeLraGuardShadow(
  user: ChatGPTUser,
  input: RunTrackGVideoOneStage12CodecSafeLraGuardShadowInput,
) {
  if (input.ownerApprovalText
    !== STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_OWNER_APPROVAL_TEXT) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_OWNER_APPROVAL_REQUIRED");
  }
  if (input.objective.trim().length < 12 || input.objective.trim().length > 500
    || !input.callbackUrl.startsWith("https://")
    || !input.objectAccessUrl.startsWith("https://")) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_INPUT_INVALID");
  }
  const existing = await latestStage12CodecSafeLraGuardShadowJob();
  if (existing) {
  ...
  if (!source.eligible || !source.source || !source.historicalFailure
    || !source.diagnosticReplayJob || !source.diagnosticReplayEvidence
    || !source.parentShadowJob || !source.parentShadowEvidence
    || !source.parentShadowResult || !source.anchorReference
    || !source.highBracketReference || !source.source.correctedPreMasterR2Key
    || !source.source.correctedPreMasterSha256
    || !source.source.correctedPreMasterByteLength || !source.source.receiptSha256) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_SOURCE_NOT_ELIGIBLE");
  }
  const health = await readStage12MediaWorkerHealth();
  const prepared = await prepareStage12MediaRequest(3);
  const controllerPolicy = STAGE12_CODEC_SAFE_LRA_GUARD_CONTROLLER_POLICY;
  const fingerprints = stage12CodecSafeLraGuardFingerprints(
    prepared.payload, controllerPolicy,
  );
  if (fingerprints.thresholdSnapshotSha256
    !== source.parentShadowResult.thresholdSnapshotSha256) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_THRESHOLD_DRIFT");
  }
  const parentRuntimeProvenance = source.parentShadowResult.runtimeProvenance;
  const parentRenderRuntimeFingerprint = sha256(new TextEncoder().encode(canonicalize({
    renderKernelFingerprint: fingerprints.renderKernelFingerprint,
    runtimeProvenance: parentRuntimeProvenance,
  })));
  const key = stage12CodecSafeLraGuardShadowIdempotencyKey(
    source.source.correctedPreMasterSha256, source.source.receiptSha256,
    source.parentShadowEvidence.id, health.imageDigest, fingerprints.algorithmFingerprint,
  );
  const token = stage12CodecSafeLraGuardShadowCallbackToken(key);
  const id = `stage12_codec_safe_lra_guard_shadow_${source.parentShadowEvidence.id}`;
  const now = new Date().toISOString();
  await getD1().batch([
    getD1().prepare(`INSERT INTO command_log
      (id,command_type,payload_json,idempotency_key,actor_identity,prev_state,next_state,
       trace_id,created_at)
      VALUES (?,'RUN_TRACK_G_VIDEO_1_STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_REPLAY',?,?,?,
       'TRACK_G_VIDEO_1_STAGE_12_CODEC_SAFE_SHADOW_FAIL',
       'TRACK_G_VIDEO_1_STAGE_12_CODEC_SAFE_LRA_GUARD_SHADOW_PENDING',?,?)`).bind(
      crypto.randomUUID(), canonicalize({ objective: input.objective.trim(),
        evidenceSemantics: "CODEC_SAFE_LRA_GUARD_SHADOW_NOT_CORRECTION",
        sourceAttemptOrdinal: 3, sourceCorrectionOrdinal: 2,
        historicalFailureCorrectionOrdinal: 3,
        parentShadowJobId: source.parentShadowJob.id,
        parentShadowEvidenceId: source.parentShadowEvidence.id,
        sourcePreMasterSha256: source.source.correctedPreMasterSha256,
        expectedWorkerImageDigest: health.imageDigest,
        parentWorkerImageDigest: source.parentShadowResult.workerImageDigest,
        ...fingerprints, correctedOutputUploaded: false, historicalBackfill: false,
        providerCallCount: 0, providerDispatch: "OFF", calibration: false,
        finalize: false, releaseEligible: false, productionActivation: false,
        autoPublish: "OFF" }), key, user.email.toLowerCase(), crypto.randomUUID(), now),
    getD1().prepare(`INSERT INTO stage12_codec_safe_lra_guard_shadow_job
      (id,stage12_job_id,source_correction_job_id,historical_failure_job_id,
       diagnostic_replay_job_id,diagnostic_replay_evidence_id,parent_shadow_job_id,
       ...
       parent_render_runtime_fingerprint,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'CODEC_SAFE_LRA_GUARD_SHADOW_NOT_CORRECTION',
       ?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, source.source.stage12JobId, source.source.id,
      source.historicalFailure.id, source.diagnosticReplayJob.id,
      source.diagnosticReplayEvidence.id, source.parentShadowJob.id,
      source.parentShadowEvidence.id, key, sha256(new TextEncoder().encode(token)),
      user.email.toLowerCase(), input.ownerApprovalText, "PENDING",
      source.source.correctedPreMasterR2Key, source.source.correctedPreMasterSha256,
      source.source.correctedPreMasterByteLength, source.source.receiptSha256,
      health.imageDigest, source.parentShadowResult.workerImageDigest,
      fingerprints.algorithmFingerprint, fingerprints.thresholdSnapshotSha256,
      fingerprints.controllerPolicySha256, fingerprints.renderKernelFingerprint,
      parentRenderRuntimeFingerprint, now, now),
  ]);
  try {
    await dispatchStage12CodecSafeLraGuardShadowReplay({
      ...prepared.payload,
      idempotencyKey: key,
      objectAccess: { url: input.objectAccessUrl, token },
      callback: { url: input.callbackUrl, token },
      codecSafeLraGuardShadowReplay: {
        ...
        diagnosticReplayJobId: source.diagnosticReplayJob.id,
        diagnosticReplayEvidenceId: source.diagnosticReplayEvidence.id,
        codecSafeTruePeakShadowJobId: source.parentShadowJob.id,
        codecSafeTruePeakShadowEvidenceId: source.parentShadowEvidence.id,
        sourceCorrectedPreMaster: { r2Key: source.source.correctedPreMasterR2Key,
          sha256: source.source.correctedPreMasterSha256,
          byteLength: source.source.correctedPreMasterByteLength },
        sourceCorrectionReceiptSha256: source.source.receiptSha256,
        parentWorkerImageDigest: source.parentShadowResult.workerImageDigest,
        parentAlgorithmFingerprint: source.parentShadowResult.algorithmFingerprint,
        parentThresholdSnapshotSha256: source.parentShadowResult.thresholdSnapshotSha256,
        parentLosslessReference: source.parentShadowResult.losslessReference,
        parentRuntimeProvenance,
        anchorReference: source.anchorReference,
        highBracketReference: source.highBracketReference,
        controllerPolicy,
        expectedWorkerImageDigest: health.imageDigest,
        ...fingerprints,
    ...
    await getD1().prepare(`UPDATE stage12_codec_safe_lra_guard_shadow_job
      SET state='FAILED',error_code=?,updated_at=? WHERE id=? AND state='PENDING'`).bind(
      error instanceof Error ? error.message.slice(0, 160)
        : "STAGE12_CODEC_SAFE_LRA_GUARD_SHADOW_START_FAILED",
      new Date().toISOString(), id,
    ).run();
    throw error;
  }
...
function stage12CodecSafeLraFeasibilitySearchIdempotencyKey(
  sourceSha256: string,
  sourceReceiptSha256: string,
  parentTruePeakEvidenceId: string,
  parentLraGuardEvidenceId: string,
  expectedWorkerImageDigest: string,
  algorithmFingerprint: string,
) {
  return createHash("sha256").update(
    ["RUN_TRACK_G_VIDEO_1_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH",
      sourceSha256, sourceReceiptSha256, parentTruePeakEvidenceId,
      parentLraGuardEvidenceId,
      expectedWorkerImageDigest, algorithmFingerprint].join("\0"),
  ).digest("hex");
}

function stage12CodecSafeLraFeasibilitySearchCallbackToken(idempotencyKey: string) {
  const signingKey = getFactoryEnv().MEDIA_REQUEST_SIGNING_KEY;
  if (!signingKey) throw new Error("MEDIA_REQUEST_SIGNING_KEY_UNAVAILABLE");
  return createHash("sha256").update(
    `STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_CALLBACK\0${idempotencyKey}\0${signingKey}`,
  ).digest("hex");
}

async function latestStage12CodecSafeLraFeasibilitySearchJob() {
  const [job] = await getDb().select().from(stage12CodecSafeLraFeasibilitySearchJobs)
    .orderBy(desc(stage12CodecSafeLraFeasibilitySearchJobs.createdAt)).limit(1);
  return job ?? null;
}

async function readStage12CodecSafeLraFeasibilitySearchEvidence(searchJobId: string) {
  const [evidence] = await getDb().select().from(stage12CodecSafeLraFeasibilitySearchEvidence)
    .where(eq(stage12CodecSafeLraFeasibilitySearchEvidence.searchJobId, searchJobId)).limit(1);
  return evidence ?? null;
}

async function stage12CodecSafeLraFeasibilitySearchSource() {
  const lineage = await stage12CodecSafeLraGuardShadowSource();
  const parentLraGuardJob = await latestStage12CodecSafeLraGuardShadowJob();
  const parentLraGuardEvidence = parentLraGuardJob
    ? await readStage12CodecSafeLraGuardShadowEvidence(parentLraGuardJob.id) : null;
  const parentLraGuardResult = parentLraGuardEvidence
    ? stage12CodecSafeLraGuardEvidenceResult(parentLraGuardEvidence) : null;
  const selectedParentCandidate = parentLraGuardResult?.candidates.find(
    (candidate) => candidate.candidatePass === 5,
  ) ?? null;
  const eligible = Boolean(lineage.eligible && lineage.source && lineage.historicalFailure
    && lineage.diagnosticReplayJob && lineage.diagnosticReplayEvidence
    && lineage.parentShadowJob && lineage.parentShadowEvidence && lineage.parentShadowResult
    && parentLraGuardJob && parentLraGuardEvidence && parentLraGuardResult
    && lineage.bootstrap.run.currentStep === "STAGE_12_READY"
    && lineage.source.correctedPreMasterSha256
      === STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SOURCE_SHA256
    && parentLraGuardEvidence.id
      === STAGE12_CODEC_SAFE_LRA_FEASIBILITY_PARENT_GUARD_EVIDENCE_ID
    && parentLraGuardEvidence.parentShadowEvidenceId
      === STAGE12_CODEC_SAFE_LRA_FEASIBILITY_PARENT_TRUE_PEAK_EVIDENCE_ID
    && parentLraGuardResult.parentShadow.evidenceId
      === STAGE12_CODEC_SAFE_LRA_FEASIBILITY_PARENT_TRUE_PEAK_EVIDENCE_ID
    && parentLraGuardJob.state === "READY" && parentLraGuardJob.shadowOutcome === "FAIL"
    && parentLraGuardJob.terminalReason === "BUDGET_EXHAUSTED"
    && parentLraGuardJob.selectedCandidatePass === 5
    && parentLraGuardResult.shadowOutcome === "FAIL"
    && parentLraGuardResult.terminalReason === "BUDGET_EXHAUSTED"
    && parentLraGuardResult.selectedCandidatePass === 5
    && parentLraGuardResult.candidates.length
      === STAGE12_CODEC_SAFE_LRA_GUARD_CONTROLLER_POLICY.maxCandidateCount
    && selectedParentCandidate?.integratedLufs === -15.25
    && selectedParentCandidate.truePeakDbtp === -1.06
    && selectedParentCandidate.loudnessRangeLu === 3.2
    && selectedParentCandidate.failedPredicates.length === 2
    && selectedParentCandidate.failedPredicates[0] === "INTEGRATED_LUFS_BELOW_MIN"
    && selectedParentCandidate.failedPredicates[1] === "LOUDNESS_RANGE_LU_BELOW_MIN"
    && parentLraGuardJob.stage12JobId === lineage.source.stage12JobId
    && parentLraGuardJob.sourceCorrectionJobId === lineage.source.id
    && parentLraGuardJob.historicalFailureJobId === lineage.historicalFailure.id
    && parentLraGuardJob.diagnosticReplayJobId === lineage.diagnosticReplayJob.id
    && parentLraGuardJob.diagnosticReplayEvidenceId === lineage.diagnosticReplayEvidence.id
    && parentLraGuardJob.parentShadowJobId === lineage.parentShadowJob.id
    && parentLraGuardJob.parentShadowEvidenceId === lineage.parentShadowEvidence.id
    && parentLraGuardEvidence.sourcePreMasterSha256
      === STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SOURCE_SHA256
    && parentLraGuardEvidence.sourcePreMasterSha256
      === lineage.source.correctedPreMasterSha256
    && parentLraGuardEvidence.sourceReceiptSha256 === lineage.source.receiptSha256
    && parentLraGuardEvidence.workerImageDigest
      === parentLraGuardEvidence.expectedWorkerImageDigest
    && parentLraGuardEvidence.correctedOutputUploaded === 0
    && parentLraGuardEvidence.historicalBackfill === 0
    && parentLraGuardEvidence.providerCallCount === 0
    && parentLraGuardEvidence.providerDispatch === "OFF"
    && parentLraGuardEvidence.calibrationExecuted === 0
    && parentLraGuardEvidence.finalizeExecuted === 0
    && parentLraGuardEvidence.releaseEligible === 0
    && parentLraGuardEvidence.productionActivationExecuted === 0
    && parentLraGuardEvidence.autoPublish === "OFF");
  return { ...lineage, parentLraGuardJob, parentLraGuardEvidence,
    parentLraGuardResult, selectedParentCandidate, eligible };
}

function stage12CodecSafeLraFeasibilityEvidenceResult(
  evidence: NonNullable<Awaited<ReturnType<
    typeof readStage12CodecSafeLraFeasibilitySearchEvidence
  >>>,
) {
  return parseStage12CodecSafeLraFeasibilitySearchResult({
    accepted: true,
    schemaVersion: 1,
    evidenceSemantics: evidence.evidenceSemantics,
    ...
    diagnosticReplay: { jobId: evidence.diagnosticReplayJobId,
      evidenceId: evidence.diagnosticReplayEvidenceId },
    parentTruePeakShadow: { jobId: evidence.parentTruePeakShadowJobId,
      evidenceId: evidence.parentTruePeakShadowEvidenceId },
    parentLraGuard: { jobId: evidence.parentLraGuardShadowJobId,
      evidenceId: evidence.parentLraGuardShadowEvidenceId },
    losslessReference: { sha256: evidence.losslessReferenceSha256,
      byteLength: evidence.losslessReferenceByteLength,
      audioFrameMd5Sha256: evidence.losslessReferenceFrameMd5Sha256,
      codec: evidence.losslessReferenceCodec,
      sampleRateHz: evidence.losslessReferenceSampleRateHz },
    parentGuardTrace: JSON.parse(evidence.parentGuardTraceJson) as unknown,
    controllerPolicy: JSON.parse(evidence.controllerPolicyJson) as unknown,
    candidates: JSON.parse(evidence.candidatesJson) as unknown,
    budgetLedger: JSON.parse(evidence.budgetLedgerJson) as unknown,
    lastEvaluatedCandidateOrdinal: evidence.lastCandidateOrdinal,
    selectedSeedOrdinal: evidence.selectedSeedOrdinal,
    selectedCandidateOrdinal: evidence.selectedCandidateOrdinal,
    verifiedCandidateOrdinal: evidence.verifiedCandidateOrdinal,
    safeRollback: JSON.parse(evidence.safeRollbackJson) as unknown,
    terminalReason: evidence.terminalReason,
    finalMeasurements: { integratedLufs: evidence.finalIntegratedLufs,
      integratedLufsExact: evidence.finalIntegratedLufsExact,
      truePeakDbtp: evidence.finalTruePeakDbtp,
    ...
    controllerPolicySha256: evidence.controllerPolicySha256,
    renderKernelFingerprint: evidence.renderKernelFingerprint,
    parentRenderKernelFingerprint: evidence.parentRenderKernelFingerprint,
    parentRenderRuntimeFingerprint: evidence.parentRenderRuntimeFingerprint,
    renderRuntimeFingerprint: evidence.renderRuntimeFingerprint,
    parentRuntimeProvenance: JSON.parse(evidence.parentRuntimeProvenanceJson) as unknown,
    runtimeProvenance: JSON.parse(evidence.runtimeProvenanceJson) as unknown,
...
export async function diagnoseTrackGVideoOneStage12CodecSafeLraFeasibilitySearch() {
  const source = await stage12CodecSafeLraFeasibilitySearchSource();
  const job = await latestStage12CodecSafeLraFeasibilitySearchJob();
  const evidence = job
    ? await readStage12CodecSafeLraFeasibilitySearchEvidence(job.id) : null;
  const result = evidence ? stage12CodecSafeLraFeasibilityEvidenceResult(evidence) : null;
  if (job?.state === "READY" && (!result
    || result.workerImageDigest !== job.workerImageDigest
    || result.shadowOutcome !== job.shadowOutcome
    || result.terminalReason !== job.terminalReason
    || result.candidates.at(-1)?.candidateOrdinal !== job.lastCandidateOrdinal
    || result.selectedSeedOrdinal !== job.selectedSeedOrdinal
    || result.selectedCandidateOrdinal !== job.selectedCandidateOrdinal
    || result.verifiedCandidateOrdinal !== job.verifiedCandidateOrdinal)) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_READ_BACK_FAILED");
  }
  return {
    shadowState: job?.state ?? (source.eligible ? "ELIGIBLE" : "BLOCKED"),
    errorCode: job?.errorCode ?? (source.eligible ? null
      : "STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_SOURCE_NOT_ELIGIBLE"),
    currentStep: source.bootstrap.run.currentStep,
    sourceAttemptOrdinal: 3 as const,
    sourceCorrectionOrdinal: 2 as const,
    historicalFailureCorrectionOrdinal: 3 as const,
    ...
    diagnosticReplayEvidenceId: job?.diagnosticReplayEvidenceId
      ?? source.diagnosticReplayEvidence?.id ?? null,
    parentTruePeakShadowJobId: job?.parentTruePeakShadowJobId
      ?? source.parentShadowJob?.id ?? null,
    parentTruePeakShadowEvidenceId: job?.parentTruePeakShadowEvidenceId
      ?? source.parentShadowEvidence?.id ?? null,
    parentLraGuardShadowJobId: job?.parentLraGuardShadowJobId
      ?? source.parentLraGuardJob?.id ?? null,
    parentLraGuardShadowEvidenceId: job?.parentLraGuardShadowEvidenceId
      ?? source.parentLraGuardEvidence?.id ?? null,
    sourcePreMasterSha256: job?.sourcePreMasterSha256
      ?? source.source?.correctedPreMasterSha256 ?? null,
    expectedWorkerImageDigest: job?.expectedWorkerImageDigest ?? null,
    parentWorkerImageDigest: job?.parentWorkerImageDigest
      ?? source.parentLraGuardEvidence?.workerImageDigest ?? null,
    workerImageDigest: job?.workerImageDigest ?? null,
    shadowOutcome: job?.shadowOutcome ?? null,
    terminalReason: job?.terminalReason ?? null,
    lastCandidateOrdinal: job?.lastCandidateOrdinal ?? null,
    selectedSeedOrdinal: job?.selectedSeedOrdinal ?? null,
    selectedCandidateOrdinal: job?.selectedCandidateOrdinal ?? null,
    verifiedCandidateOrdinal: job?.verifiedCandidateOrdinal ?? null,
    result,
...
function requireStage12CodecSafeLraFeasibilitySearchToken(
  job: Awaited<ReturnType<typeof latestStage12CodecSafeLraFeasibilitySearchJob>>,
  idempotencyKey: string,
  token: string,
): asserts job is NonNullable<typeof job> {
  if (!job || job.idempotencyKey !== idempotencyKey || !HEX64.test(token)
...
export async function readTrackGVideoOneStage12CodecSafeLraFeasibilitySearchSource(
  idempotencyKey: string,
  token: string,
  expectedSha256: string,
) {
  const job = await latestStage12CodecSafeLraFeasibilitySearchJob();
  requireStage12CodecSafeLraFeasibilitySearchToken(job, idempotencyKey, token);
  if (job.state !== "PENDING"
    || expectedSha256 !== STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SOURCE_SHA256
    || expectedSha256 !== job.sourcePreMasterSha256) {
    throw new Error("STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_SOURCE_CONFLICT");
  }
  return readVerifiedProductionEvidence(job.sourcePreMasterR2Key, job.sourcePreMasterSha256);
}

export async function recordTrackGVideoOneStage12CodecSafeLraFeasibilitySearchCallback(input: {
  idempotencyKey: string;
  token: string;
  ...
  const job = await latestStage12CodecSafeLraFeasibilitySearchJob();
  requireStage12CodecSafeLraFeasibilitySearchToken(job, input.idempotencyKey, input.token);
  if (job.state === "READY") {
    return { accepted: true, replayed: true, jobStatus: "READY" as const };
  }
  if (job.state !== "PENDING") {
    throw new Error("STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_CALLBACK_STATE_CONFLICT");
  }
  if (!input.result) {
    const code = input.errorCode && /^[A-Z0-9_:.-]{1,160}$/u.test(input.errorCode)
      ? input.errorCode : "STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_FAILED";
    await getD1().prepare(`UPDATE stage12_codec_safe_lra_feasibility_search_job
      SET state='FAILED',error_code=?,updated_at=? WHERE id=? AND state='PENDING'`)
      .bind(code, new Date().toISOString(), job.id).run();
    return { accepted: true, replayed: false, jobStatus: "FAILED" as const };
  }
  const result = parseStage12CodecSafeLraFeasibilitySearchResult(input.result);
  if (result.source.correctionJobId !== job.sourceCorrectionJobId
    || result.historicalFailure.correctionJobId !== job.historicalFailureJobId
    || result.diagnosticReplay.jobId !== job.diagnosticReplayJobId
    || result.diagnosticReplay.evidenceId !== job.diagnosticReplayEvidenceId
    || result.parentTruePeakShadow.jobId !== job.parentTruePeakShadowJobId
    || result.parentTruePeakShadow.evidenceId !== job.parentTruePeakShadowEvidenceId
    || result.parentLraGuard.jobId !== job.parentLraGuardShadowJobId
    || result.parentLraGuard.evidenceId !== job.parentLraGuardShadowEvidenceId
    || result.source.sha256 !== STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SOURCE_SHA256
    || result.source.sha256 !== job.sourcePreMasterSha256
    || result.source.receiptSha256 !== job.sourceReceiptSha256
    || result.expectedWorkerImageDigest !== job.expectedWorkerImageDigest
    || result.parentWorkerImageDigest !== job.parentWorkerImageDigest
    ...
    || result.controllerPolicySha256 !== job.controllerPolicySha256
    || result.renderKernelFingerprint !== job.renderKernelFingerprint
    || result.parentRenderKernelFingerprint !== job.parentRenderKernelFingerprint
    || result.parentRenderRuntimeFingerprint !== job.parentRenderRuntimeFingerprint
    || result.renderRuntimeFingerprint !== job.parentRenderRuntimeFingerprint) {
    throw new Error("STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_RESULT_LINEAGE_INVALID");
  }
  const evidenceId = sha256(new TextEncoder().encode(canonicalize({
    searchJobId: job.id, result,
  })));
  const lastCandidateOrdinal = result.candidates.at(-1)?.candidateOrdinal;
  if (lastCandidateOrdinal === undefined) {
    throw new Error("STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_RESULT_EMPTY");
  }
  const now = new Date().toISOString();
  await getD1().batch([
    getD1().prepare(`INSERT INTO stage12_codec_safe_lra_feasibility_search_evidence
      (id,search_job_id,stage12_job_id,source_correction_job_id,historical_failure_job_id,
       ...
       parent_lra_guard_shadow_evidence_id,evidence_semantics,lossless_reference_sha256,
       lossless_reference_byte_length,lossless_reference_frame_md5_sha256,
       lossless_reference_codec,lossless_reference_sample_rate_hz,parent_guard_trace_json,
       controller_policy_json,candidates_json,budget_ledger_json,last_candidate_ordinal,
       selected_seed_ordinal,
       selected_candidate_ordinal,verified_candidate_ordinal,safe_rollback_json,terminal_reason,
       final_integrated_lufs,
       final_integrated_lufs_exact,final_true_peak_dbtp,final_true_peak_dbtp_exact,
       final_loudness_range_lu,final_loudness_range_lu_exact,failed_predicates_json,
       shadow_outcome,expected_worker_image_digest,parent_worker_image_digest,
       worker_image_digest,algorithm_fingerprint,threshold_snapshot_sha256,
       controller_policy_sha256,render_kernel_fingerprint,parent_render_kernel_fingerprint,
       parent_render_runtime_fingerprint,
       render_runtime_fingerprint,parent_runtime_provenance_json,runtime_provenance_json,
       source_pre_master_r2_key,source_pre_master_sha256,source_pre_master_byte_length,
      ...
      evidenceId, job.id, job.stage12JobId, job.sourceCorrectionJobId,
      job.historicalFailureJobId, job.diagnosticReplayJobId,
      job.diagnosticReplayEvidenceId, job.parentTruePeakShadowJobId,
      job.parentTruePeakShadowEvidenceId, job.parentLraGuardShadowJobId,
      job.parentLraGuardShadowEvidenceId, result.evidenceSemantics,
      result.losslessReference.sha256, result.losslessReference.byteLength,
      result.losslessReference.audioFrameMd5Sha256,
      result.losslessReference.sampleRateHz, canonicalize(result.parentGuardTrace),
      canonicalize(result.controllerPolicy), canonicalize(result.candidates),
      canonicalize(result.budgetLedger), lastCandidateOrdinal, result.selectedSeedOrdinal,
      result.selectedCandidateOrdinal, result.verifiedCandidateOrdinal,
      canonicalize(result.safeRollback),
      result.terminalReason, result.finalMeasurements.integratedLufs,
      result.finalMeasurements.integratedLufsExact, result.finalMeasurements.truePeakDbtp,
      result.finalMeasurements.truePeakDbtpExact, result.finalMeasurements.loudnessRangeLu,
      result.finalMeasurements.loudnessRangeLuExact, canonicalize(result.failedPredicates),
      result.shadowOutcome, result.expectedWorkerImageDigest, result.parentWorkerImageDigest,
      result.workerImageDigest, result.algorithmFingerprint, result.thresholdSnapshotSha256,
      result.controllerPolicySha256, result.renderKernelFingerprint,
      result.parentRenderKernelFingerprint,
      result.parentRenderRuntimeFingerprint, result.renderRuntimeFingerprint,
      canonicalize(result.parentRuntimeProvenance), canonicalize(result.runtimeProvenance),
      result.source.r2Key, result.source.sha256, result.source.byteLength,
      result.source.receiptSha256),
    getD1().prepare(`UPDATE stage12_codec_safe_lra_feasibility_search_job SET state='READY',
      shadow_outcome=?,terminal_reason=?,last_candidate_ordinal=?,selected_seed_ordinal=?,
      selected_candidate_ordinal=?,verified_candidate_ordinal=?,worker_image_digest=?,
      render_runtime_fingerprint=?,
      updated_at=? WHERE id=? AND state='PENDING'`).bind(
      result.shadowOutcome, result.terminalReason, lastCandidateOrdinal,
      result.selectedSeedOrdinal, result.selectedCandidateOrdinal,
      result.verifiedCandidateOrdinal, result.workerImageDigest,
      result.renderRuntimeFingerprint, now, job.id),
  ]);
  return { accepted: true, replayed: false, jobStatus: "READY" as const };
}

export async function runTrackGVideoOneStage12CodecSafeLraFeasibilitySearch(
  user: ChatGPTUser,
  input: RunTrackGVideoOneStage12CodecSafeLraFeasibilitySearchInput,
) {
  if (input.ownerApprovalText
    !== STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_OWNER_APPROVAL_TEXT) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_OWNER_APPROVAL_REQUIRED");
  }
  if (input.objective.trim().length < 12 || input.objective.trim().length > 500
    || !input.callbackUrl.startsWith("https://")
    || !input.objectAccessUrl.startsWith("https://")) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_INPUT_INVALID");
  }
  const existing = await latestStage12CodecSafeLraFeasibilitySearchJob();
  if (existing) {
    return { ...(await diagnoseTrackGVideoOneStage12CodecSafeLraFeasibilitySearch()),
      replayed: true };
  }
  const source = await stage12CodecSafeLraFeasibilitySearchSource();
  if (!source.eligible || !source.source || !source.historicalFailure
    || !source.diagnosticReplayJob || !source.diagnosticReplayEvidence
    || !source.parentShadowJob || !source.parentShadowEvidence
    || !source.parentLraGuardJob || !source.parentLraGuardEvidence
    || !source.parentLraGuardResult || !source.selectedParentCandidate
    || !source.source.correctedPreMasterR2Key || !source.source.correctedPreMasterSha256
    || !source.source.correctedPreMasterByteLength || !source.source.receiptSha256) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_SOURCE_NOT_ELIGIBLE");
  }
  const health = await readStage12MediaWorkerHealth();
  if (health.codecSafeLraFeasibilitySearchReady !== true) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_WORKER_NOT_READY");
  }
  const prepared = await prepareStage12MediaRequest(3);
  const controllerPolicy = STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_CONTROLLER_POLICY;
  const fingerprints = stage12CodecSafeLraFeasibilitySearchFingerprints(
    prepared.payload, controllerPolicy,
  );
  if (fingerprints.thresholdSnapshotSha256
      !== source.parentLraGuardResult.thresholdSnapshotSha256
    || fingerprints.renderKernelFingerprint
      !== source.parentLraGuardResult.renderKernelFingerprint) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_PARENT_DRIFT");
  }
  const parentRuntimeProvenance = source.parentLraGuardResult.runtimeProvenance;
  const parentRenderRuntimeFingerprint = sha256(new TextEncoder().encode(canonicalize({
    renderKernelFingerprint: fingerprints.renderKernelFingerprint,
    runtimeProvenance: parentRuntimeProvenance,
  })));
  if (parentRenderRuntimeFingerprint !== source.parentLraGuardResult.renderRuntimeFingerprint) {
    throw new Error("TRACK_G_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_RUNTIME_DRIFT");
  }
  const key = stage12CodecSafeLraFeasibilitySearchIdempotencyKey(
    source.source.correctedPreMasterSha256, source.source.receiptSha256,
    source.parentShadowEvidence.id, source.parentLraGuardEvidence.id,
    health.imageDigest, fingerprints.algorithmFingerprint,
  );
  const token = stage12CodecSafeLraFeasibilitySearchCallbackToken(key);
  const id = `stage12_codec_safe_lra_feasibility_${source.parentLraGuardEvidence.id}`;
  const now = new Date().toISOString();
  await getD1().batch([
    getD1().prepare(`INSERT INTO command_log
      (id,command_type,payload_json,idempotency_key,actor_identity,prev_state,next_state,
      ...
      VALUES (?,'RUN_TRACK_G_VIDEO_1_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH',?,?,?,
       ...
       'TRACK_G_VIDEO_1_STAGE_12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_PENDING',?,?)`).bind(
      crypto.randomUUID(), canonicalize({ objective: input.objective.trim(),
        evidenceSemantics: "CODEC_SAFE_LRA_FEASIBILITY_SHADOW_NOT_CORRECTION",
        sourceAttemptOrdinal: 3, sourceCorrectionOrdinal: 2,
        historicalFailureCorrectionOrdinal: 3,
        sourcePreMasterSha256: STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SOURCE_SHA256,
        parentTruePeakShadowEvidenceId:
          STAGE12_CODEC_SAFE_LRA_FEASIBILITY_PARENT_TRUE_PEAK_EVIDENCE_ID,
        parentLraGuardShadowEvidenceId:
          STAGE12_CODEC_SAFE_LRA_FEASIBILITY_PARENT_GUARD_EVIDENCE_ID,
        expectedWorkerImageDigest: health.imageDigest, parentWorkerImageDigest:
          source.parentLraGuardResult.workerImageDigest, controllerPolicy,
        ...fingerprints, correctedOutputUploaded: false, historicalBackfill: false,
        providerCallCount: 0, providerDispatch: "OFF", calibration: false,
        finalize: false, releaseEligible: false, productionActivation: false,
        autoPublish: "OFF" }), key, user.email.toLowerCase(), crypto.randomUUID(), now),
    getD1().prepare(`INSERT INTO stage12_codec_safe_lra_feasibility_search_job
      (id,stage12_job_id,source_correction_job_id,historical_failure_job_id,
       diagnostic_replay_job_id,diagnostic_replay_evidence_id,parent_true_peak_shadow_job_id,
       parent_true_peak_shadow_evidence_id,parent_lra_guard_shadow_job_id,
       ...
       expected_worker_image_digest,parent_worker_image_digest,algorithm_fingerprint,
       threshold_snapshot_sha256,controller_policy_sha256,render_kernel_fingerprint,
       parent_render_kernel_fingerprint,parent_render_runtime_fingerprint,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'CODEC_SAFE_LRA_FEASIBILITY_SHADOW_NOT_CORRECTION',
       ?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, source.source.stage12JobId, source.source.id,
      source.historicalFailure.id, source.diagnosticReplayJob.id,
      source.diagnosticReplayEvidence.id, source.parentShadowJob.id,
      source.parentShadowEvidence.id, source.parentLraGuardJob.id,
      source.parentLraGuardEvidence.id, key, sha256(new TextEncoder().encode(token)),
      user.email.toLowerCase(), input.ownerApprovalText, "PENDING",
      source.source.correctedPreMasterR2Key, source.source.correctedPreMasterSha256,
      source.source.correctedPreMasterByteLength, source.source.receiptSha256,
      health.imageDigest, source.parentLraGuardResult.workerImageDigest,
      fingerprints.algorithmFingerprint, fingerprints.thresholdSnapshotSha256,
      fingerprints.controllerPolicySha256, fingerprints.renderKernelFingerprint,
      source.parentLraGuardResult.renderKernelFingerprint,
      parentRenderRuntimeFingerprint, now, now),
  ...
  try {
    await dispatchStage12CodecSafeLraFeasibilitySearch({
      ...
      codecSafeLraFeasibilitySearch: {
        ...
        codecSafeTruePeakShadowJobId: source.parentShadowJob.id,
        codecSafeTruePeakShadowEvidenceId: source.parentShadowEvidence.id,
        codecSafeLraGuardShadowJobId: source.parentLraGuardJob.id,
        codecSafeLraGuardShadowEvidenceId: source.parentLraGuardEvidence.id,
        sourceCorrectedPreMaster: { r2Key: source.source.correctedPreMasterR2Key,
          sha256: source.source.correctedPreMasterSha256,
          byteLength: source.source.correctedPreMasterByteLength },
        sourceCorrectionReceiptSha256: source.source.receiptSha256,
        parentWorkerImageDigest: source.parentLraGuardResult.workerImageDigest,
        parentAlgorithmFingerprint: source.parentLraGuardResult.algorithmFingerprint,
        parentThresholdSnapshotSha256: source.parentLraGuardResult.thresholdSnapshotSha256,
        parentControllerPolicySha256: source.parentLraGuardResult.controllerPolicySha256,
        parentRenderKernelFingerprint: source.parentLraGuardResult.renderKernelFingerprint,
        parentRenderRuntimeFingerprint,
        parentLosslessReference: source.parentLraGuardResult.losslessReference,
        parentRuntimeProvenance,
        parentGuardTrace: {
          shadowOutcome: "FAIL" as const,
          terminalReason: "BUDGET_EXHAUSTED" as const,
          lastEvaluatedCandidatePass: 7 as const,
          bestSafeCandidatePass: 5 as const,
          selectedCandidatePass: 5 as const,
          finalMeasurements: source.parentLraGuardResult.finalMeasurements,
          failedPredicates: source.parentLraGuardResult.failedPredicates,
          candidates: source.parentLraGuardResult.candidates,
        },
        controllerPolicy,
        expectedWorkerImageDigest: health.imageDigest,
        ...fingerprints,
    ...
    await getD1().prepare(`UPDATE stage12_codec_safe_lra_feasibility_search_job
      SET state='FAILED',error_code=?,updated_at=? WHERE id=? AND state='PENDING'`).bind(
      error instanceof Error ? error.message.slice(0, 160)
        : "STAGE12_CODEC_SAFE_LRA_FEASIBILITY_SEARCH_START_FAILED",
      new Date().toISOString(), id,
    ).run();
    throw error;
  }
...
async function finalizeTrackGVideoOneStage12Prepared(
  ...
  const audioP0Correction = await latestStage12AudioP0CorrectionJob();
  const useAudioP0Correction = prepared.job.state === "FAILED"
    && audioP0Correction?.state === "READY" && audioP0Correction.outcome === "PASS"
    && Boolean(audioP0Correction.receiptR2Key && audioP0Correction.receiptSha256);
  const useCorrection = !useAudioP0Correction && prepared.job.state === "FAILED" && correction?.state === "READY"
    && correction.outcome === "PASS" && Boolean(correction.receiptR2Key && correction.receiptSha256);
  if (prepared.job.state === "FAILED" && !useAudioP0Correction && !useCorrection) {
    throw new Error(`TRACK_G_STAGE_12_JOB_FAILED:${prepared.job.errorCode ?? "UNKNOWN"}`);
  }
  const workerReceiptR2Key = useAudioP0Correction ? audioP0Correction!.receiptR2Key
    : useCorrection ? correction!.receiptR2Key : prepared.job.receiptR2Key;
  const workerReceiptSha256 = useAudioP0Correction ? audioP0Correction!.receiptSha256
    : useCorrection ? correction!.receiptSha256 : prepared.job.receiptSha256;
  const expectedReceiptIdempotencyKey = useAudioP0Correction ? audioP0Correction!.idempotencyKey
    : useCorrection ? correction!.idempotencyKey : prepared.workerIdempotencyKey;
  ...
  const artifactEnvelope = {
    ...
    provenance: {
      ...
      workerReceiptSha256,
      ...(useAudioP0Correction ? {
        correctedPreMasterJobId: audioP0Correction!.predecessorCorrectedPreMasterJobId,
        audioP0CorrectionJobId: audioP0Correction!.id,
        sourcePreMasterSha256: audioP0Correction!.sourcePreMasterSha256,
        correctionOrdinal: audioP0Correction!.correctionOrdinal,
      } : {}),
