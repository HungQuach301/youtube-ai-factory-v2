...
import {
  ...
  diagnoseTrackGVideoOneStage12CodecSafeLraFeasibilitySearch,
  ...
  runTrackGVideoOneStage12CodecSafeLraFeasibilitySearch,
  ...
  server.registerTool(
    ...
    async ({ commandType, trackCode, videoNumber, stageCode, attemptOrdinal }) => {
      ...
      if (commandType === "RECOVER_STAGE_12_ATTEMPT_3") {
        ...
        if (attemptOrdinal !== 3) throw new Error("STABLE_COMMAND_ATTEMPT_MISMATCH");
        diagnostic = (await diagnoseTrackGVideoOneStage12CodecSafeLraFeasibilitySearch()) as unknown as Record<string, unknown>;
  ...
  server.registerTool(
    ...
    async ({ commandType, trackCode, videoNumber, stageCode, attemptOrdinal,
      ...
      if (commandType === "RECOVER_STAGE_12_ATTEMPT_3") {
        ...
        const shadow = await runTrackGVideoOneStage12CodecSafeLraFeasibilitySearch(user, {
          objective,
          ownerApprovalText,
          callbackUrl: shadowRoute,
          objectAccessUrl: shadowRoute,
        ...
        result = { replayed: shadow.replayed, runId,
          ...
          receipt: { sourceAttemptOrdinal: 3, sourceCorrectionOrdinal: 2,
            historicalFailureCorrectionOrdinal: 3, shadowState: shadow.shadowState,
            shadowOutcome: shadow.shadowOutcome, terminalReason: shadow.terminalReason,
            parentTruePeakShadowEvidenceId: shadow.parentTruePeakShadowEvidenceId,
            parentLraGuardShadowEvidenceId: shadow.parentLraGuardShadowEvidenceId,
