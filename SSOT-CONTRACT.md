...
A Factory capability is `DONE` only when one vertical slice is demonstrably
complete: guarded domain executor, D1/R2 evidence, read projection, MCP/API
exposure, `/operate` presentation or owner action, integration tests and a
Production read-back. An executor that is callable only through MCP is
`IMPLEMENTED_HEADLESS`, not `DONE`.

Prefer the smallest slice through existing domain services and existing tables.
Do not add a command framework, projection framework, queue or database table
unless a concrete requirement cannot be met safely with the current control
plane. UI and MCP must call the same executor and must preserve the same
authorization, idempotency, evidence and fail-closed rules.
