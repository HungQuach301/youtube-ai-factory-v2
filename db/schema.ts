...
export const stage12CorrectedPreMasterJobs = sqliteTable("stage12_corrected_pre_master_job", {
  ...
  idempotencyKey: text("idempotency_key").notNull(), callbackTokenHash: text("callback_token_hash").notNull(),
  actorIdentity: text("actor_identity").notNull(), ownerApprovalText: text("owner_approval_text").notNull(),
  state: text("state", { enum: ["PENDING", "READY", "FAILED"] }).notNull(),
  ...
  receiptR2Key: text("receipt_r2_key"), receiptSha256: text("receipt_sha256"),
  workerImageDigest: text("worker_image_digest"), reportSha256: text("report_sha256"),
  outcome: text("outcome", { enum: ["PASS", "FAIL"] }), failuresJson: text("failures_json"),
  measurementsJson: text("measurements_json"), providerCallCount: integer("provider_call_count").notNull().default(0),
  ...
  errorCode: text("error_code"), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
