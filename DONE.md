...
| Live calibration/deploy cần phê duyệt riêng | Input bắt buộc `owner_approval_text`; exact phrase `PROMOTE_CALIBRATED_MEDIA_WORKER` được kiểm tra ở step đầu tiên, trước checkout, credentials và provider |
...
| Workflow khôi phục `push:` hoặc bỏ manual approval | `container-contract.test.ts` → FAIL |
