...
CREATE TABLE archetype (
  ...
  min_first_pass_yield  REAL NOT NULL
);
...
-- QUALIFIED chỉ được cấp khi có run PASS tương ứng
CREATE TRIGGER trg_binding_requires_passed_run
BEFORE UPDATE OF qualification_state ON capability_archetype_binding
...
-- DOWN
-- DROP TRIGGER trg_binding_requires_passed_run; DROP TABLE gold_sample;
-- DROP TABLE qualification_run; DROP TABLE capability_archetype_binding;
-- DROP TABLE fixture; DROP TABLE archetype; DROP TABLE capability;
...
CREATE TABLE provider_request (
  ...
  request_r2_key    TEXT NOT NULL,          -- CORE-06: snapshot hai chiều bắt buộc
...
WHEN NEW.state = 'SETTLED' AND OLD.state <> 'HELD'
...
-- DROP TRIGGER trg_no_retry_terminal_errors; DROP TABLE provider_request;
